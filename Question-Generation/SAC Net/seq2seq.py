...
class TextFieldPredictor(nn.Module):
    ...
    def forward_prepro(self, input, input_masks, answer_features=None):
        ...
        if answer_features is not None:
            ...
            self.input_embeddings = torch.cat((input_embeddings, unsqueezed_answer_features), 2)  # [bs, len_c, dim+1]
        else:
            self.input_embeddings = input_embeddings

        self.lstm_embeddings = self.encoder(self.input_embeddings)  # [bs, len_c, dim+1]->[bs, len_c, dim]
...
class LPN(nn.Module):
    def __init__(self, config, stroke_embedding=None):
        ...
        if stroke_embedding is not None:
            self.stroke_embedding = Embedding.from_pretrained(torch.from_numpy(stroke_embedding), freeze=True)
        else:
    ...
    def forward(self, input_tokens, context_tokens, context_masks, answer_features, is_training=True):
        ...
        if not is_training:
            ...
            combined_predictions = reshaped_language_probs[:, :-1, :].contiguous()  # [bs, len_q-1, vocab_size]
